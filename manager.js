...
function displaySharedReportSummary(date) {
  fetchReportSummary(date, (group, sum, reports, error) => {
    if (error) {
      document.getElementById('report-table').innerHTML = '<p class="text-red-500">Lỗi khi tải báo cáo.</p>';
      return;
    }

    const table = `
      <div><strong>Số Dư Đầu Kỳ:</strong><br>${group.opening.join('<br>')}</div><hr>
      <div><strong>Số Dư Cuối Kỳ:</strong><br>${group.closing.join('<br>')}</div><hr>
      <div><strong>Chi Phí:</strong><br>${group.cost.join('<br>')}</div><hr>
      <div><strong>Doanh Thu:</strong><br>${group.revenue.join('<br>')}</div><hr>
      <div><strong>Số Tiền Thực Tế:</strong><br>${sum.real}</div><hr>

...
export function initRevenues() {
  ...
  document.getElementById('add-revenue').addEventListener('click', async () => {
    ...
    if (isNaN(amount) || amount <= 0) {
      document.getElementById('revenueErrorContainer').innerHTML = '<div class="error-message">Vui lòng nhập số tiền hợp lệ</div>';
      return;
    }
    ...
    try {
      ...
      document.getElementById('revenueErrorContainer').innerHTML = `<div class="error-message">${error.message}</div>`;
    }
  });

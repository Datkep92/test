...
export function initExports() {
  ...
  document.getElementById('add-export').addEventListener('click', async () => {
    if (!selectedProduct) {
      document.getElementById('exportErrorContainer').innerHTML = '<div class="error-message">Vui lòng chọn sản phẩm</div>';
      return;
    }

    const quantity = parseInt(document.getElementById('exportQuantity').value) || 1;
    if (quantity <= 0) {
      document.getElementById('exportErrorContainer').innerHTML = '<div class="error-message">Số lượng phải lớn hơn 0</div>';
      return;
    }
  ...
  onSnapshot(query(collection(db, 'exports'), orderBy('timestamp', 'desc')), (snapshot) => {
    ...
    snapshot.forEach((doc) => {
      ...
      const row = `<tr>
        ...
        <td>${data.quantity}</td>
        <td>${data.unit}</td>
        <td class="actions manager-only"><button class="danger" onclick="window.deleteExport('${doc.id}')">Xóa</button></td>
      </tr>`;
      table.innerHTML += row;
    });
  });

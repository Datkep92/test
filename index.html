<!DOCTYPE html>
<html lang="vi">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>📊 Quản lý Kinh doanh</title>
  <link rel="stylesheet" href="styles.css">
</head>
<body>
  <div id="appHeader" class="hidden">
    <h2>📊 Quản lý Kinh doanh</h2>
    <button id="logout-button">Đăng xuất</button>
  </div>

  <div id="loginBox">
    <h2>🔐 Đăng nhập</h2>
    <div id="loginScreen">
      <input type="text" id="username" placeholder="Tên đăng nhập"><br>
      <input type="password" id="password" placeholder="Mật khẩu"><br>
      <label><input type="checkbox" id="rememberMe"> Ghi nhớ đăng nhập</label><br>
      <button id="login-button">Đăng nhập</button>
      <div id="errorContainer"></div>
    </div>
  </div>

  <div id="saleApp" class="hidden">
    <div class="section">
      <h3>Chi phí</h3>
      <div id="expensesApp">
        <button id="add-expense">Thêm</button>
        <input type="text" id="expenseInput" placeholder="Nhập chi phí (VD: ăn 100000)"><br>
        <table id="expenseTable">
          <thead><tr><th>Mô tả</th><th>Số tiền</th><th>Hành động</th></tr></thead>
          <tbody></tbody>
        </table>
        <div id="errorContainer"></div>
      </div>
    </div>

    <div class="section">
      <h3>Xuất hàng</h3>
      <div id="exportsApp">
        <div id="productList"></div>
        <input type="number" id="exportQuantity" value="1" min="1"><br>
        <button id="add-export">Thêm xuất hàng</button>
        <table id="exportTable">
          <thead><tr><th>Sản phẩm</th><th>Số lượng</th><th>ĐVT</th><th>Hành động</th></tr></thead>
          <tbody></tbody>
        </table>
        <div id="errorContainer"></div>
      </div>
    </div>

    <div class="section">
      <h3>Doanh thu</h3>
      <div id="revenuesApp">
        <button id="add-revenue">Thêm</button>
        <input type="text" id="revenueAmount" placeholder="Nhập doanh thu"><br>
        <div id="errorContainer"></div>
      </div>
    </div>
  </div>

  <div id="adminApp" class="hidden">
    <div class="section">
      <h3>📝 Tổng kết ngày</h3>
      <div id="dailySummaryApp">
        <div>
          <span id="summaryDate"></span><br>
          Doanh thu: <span id="totalRevenue">0₫</span><br>
          Tổng chi phí: <span id="totalExpense">0₫</span><br>
          Số dư: <span id="dailyBalance">0₫</span>
        </div>
        <div id="exportSummary"></div>
        <textarea id="dailyNote" rows="3" placeholder="Ghi chú cá nhân"></textarea><br>
        <button id="save-daily">Lưu dữ liệu</button>
        <div id="errorContainer"></div>
      </div>
    </div>

    <div class="section">
      <h3>📦 Quản lý Kho hàng</h3>
      <div id="inventoryApp">
        <input type="text" id="productName" placeholder="Tên sản phẩm"><br>
        <input type="number" id="productQuantity" placeholder="Số lượng"><br>
        <input type="text" id="productUnit" placeholder="ĐVT"><br>
        <input type="number" id="productPrice" placeholder="Giá đơn vị"><br>
        <button id="add-product">Thêm sản phẩm</button>
        <table id="productTable">
          <thead><tr><th>Tên sản phẩm</th><th>Số lượng</th><th>ĐVT</th><th>Giá đơn vị</th><th>Hành động</th></tr></thead>
          <tbody></tbody>
        </table>
        <div id="errorContainer"></div>
      </div>
    </div>

    <div class="section">
      <h3>📊 Báo cáo</h3>
      <div id="reportsApp">
        <button id="daily-report">Báo cáo ngày</button>
        <button id="monthly-report">Báo cáo tháng</button>
        <button id="print-report">In báo cáo</button>
        <input type="date" id="reportDate"><br>
        <select id="reportUserFilter">
          <option value="all">Tất cả người dùng</option>
        </select>
        <select id="reportCategoryFilter">
          <option value="all">Tất cả danh mục</option>
        </select>
        <button id="apply-report">Áp dụng</button>
        <div id="reportOutput"></div>
        <div id="errorContainer"></div>
      </div>
    </div>

    <div class="section">
      <h3>👥 Quản lý Người dùng</h3>
      <div id="usersApp">
        <input type="text" id="newUsername" placeholder="Tên đăng nhập"><br>
        <input type="password" id="newPassword" placeholder="Mật khẩu"><br>
        <select id="newUserRole">
          <option value="staff">Nhân viên</option>
          <option value="admin">Quản lý</option>
        </select><br>
        <button id="add-user">Thêm người dùng</button>
        <table id="userTable">
          <thead><tr><th>Tên đăng nhập</th><th>Vai trò</th><th>Trạng thái</th><th>Hành động</th></tr></thead>
          <tbody></tbody>
        </table>
        <div id="errorContainer"></div>
      </div>
    </div>
  </div>

<script type="module">
  import { initAuth } from './js/auth.js';
  import { initExpenses } from './js/expenses.js';
  import { initExports } from './js/exports.js';
  import { initRevenues } from './js/revenues.js';
  import { initDailySummary } from './js/dailySummary.js';
  import { initInventory } from './js/inventory.js';
  import { initReports } from './js/reports.js';
  import { initUsers } from './js/users.js';
  import { deleteDoc, editProduct, deleteExport, toggleUserStatus, deleteUser } from './js/utils.js';

    document.addEventListener('DOMContentLoaded', () => {
      initAuth();
      initExpenses();
      initExports();
      initRevenues();
      initDailySummary();
      initInventory();
      initReports();
      initUsers();
      window.deleteDoc = deleteDoc;
      window.editProduct = editProduct;
      window.deleteExport = deleteExport;
      window.toggleUserStatus = toggleUserStatus;
      window.deleteUser = deleteUser;
    });
  </script>
</body>
</html>
